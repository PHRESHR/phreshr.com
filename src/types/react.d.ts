...
declare module 'react' {
	...
	type Thenable<T, R> = {
		then(resolve: (value: T) => unknown, reject: (error: unknown) => unknown): R;
	};
	...
	export function useEffect<T>(effect: () => T | (() => T), cDU?: any[]): void;

	export function useLayoutEffect(effect: () => void | (() => void), cDU?: any[]): void;

	export function useMutationEffect(effect: () => void | (() => void), cDU?: any[]): void;
